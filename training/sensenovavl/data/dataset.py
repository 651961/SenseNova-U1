...
def preprocess_pixel_values(pixel_values, patch_size=16):
    ...
    # Features may be resumed from an RGB cache while newly decoded samples
    # are RGBA.  Promote the RGB entries when the batch contains either kind
    # of alpha-bearing image so concatenation remains well-defined.  A
    # missing alpha is represented by an opaque channel (the transform's
    # alpha normalization is [0, 1]).
    channel_counts = []
    for idx, px in enumerate(pixel_values):
        if not torch.is_tensor(px) or px.ndim != 3:
            shape = tuple(getattr(px, "shape", ()))
            raise ValueError(
                f"Expected image tensor [C, H, W] at index {idx}, got {shape}."
            )
        c = int(px.shape[0])
        if c not in (3, 4):
            raise ValueError(
                f"Expected RGB/RGBA tensor for image {idx}, got {c} channels."
            )
        channel_counts.append(c)
    target_channels = 4 if 4 in channel_counts else 3

    for idx, px in enumerate(pixel_values):
        c, h, w = px.shape
        if c == 3 and target_channels == 4:
            alpha = torch.ones(
                (1, h, w),
                dtype=px.dtype,
                device=px.device,
            )
            px = torch.cat((px, alpha), dim=0)
            c = 4
        grid_h = h // patch_size
        grid_w = w // patch_size
