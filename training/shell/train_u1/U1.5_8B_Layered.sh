#!/bin/bash
# Standalone torchrun launcher for SenseNova-U1.5 8B layered image pre-training.
#
# This launcher intentionally contains the full U1.5_8B.sh configuration
# instead of invoking or inheriting that script.  Keep the two launchers
# independently editable; only the layered model/channel defaults below differ.
#
# Single-node (8 GPUs):
#   bash shell/train_u1/U1.5_8B_Layered.sh
#
# Multi-node — run on each node, replacing NODE_RANK / MASTER_ADDR:
#   NNODES=2 NODE_RANK=0 MASTER_ADDR=10.0.0.1 bash shell/train_u1/U1.5_8B_Layered.sh
#   NNODES=2 NODE_RANK=1 MASTER_ADDR=10.0.0.1 bash shell/train_u1/U1.5_8B_Layered.sh
...
export MODEL_NAME_OR_PATH=${MODEL_NAME_OR_PATH:-"/models/SenseNova-U1.5-8B-MoT-Layered"}
export VOCAB_FILE=${VOCAB_FILE:-"${MODEL_NAME_OR_PATH}"}
export TOKENIZER_PATH=${TOKENIZER_PATH:-"${MODEL_NAME_OR_PATH}"}
export mm_data_path=${mm_data_path:-"/datasets/codes_zsqiao/edit_dataset/goodcase_it2i_meta.json"}
...
export init_steps=10

# ============================ Data / sequence ============================ #
export num_imgs=144
export seq_len=25500
export max_sample_tokens=25500
...
export output_channels=${output_channels:-4}
export P_mean=-0.8
export P_std=0.8
