...
def _to_pil(batch: torch.Tensor, alpha_threshold: int | None = None) -> GeneratedImages:
    """Convert legacy image batches or bottom-to-top RGBA layer stacks to PIL."""
...
def _composite_layers(layers: Sequence[Image.Image]) -> Image.Image:
    """Alpha-composite a bottom-to-top layer stack for previews and comparisons."""
    if not layers:
    ...
    for layer_idx, layer in enumerate(layers):
        if layer.size != canvas.size:
            raise ValueError(f"Layer {layer_idx} has size {layer.size}, expected {canvas.size}.")
        canvas = Image.alpha_composite(canvas, layer.convert("RGBA"))
    return canvas.convert("RGB")


def _comparison_image(images: GeneratedImages) -> Image.Image:
    return _composite_layers(_as_layer_stacks(images)[0])
...
def parse_args() -> argparse.Namespace:
    ...
    src.add_argument(
        ...
        '"seed": int, "num_layers": int, "output_filename": str, "type": str}. '
        'When "width" and "height" are both '
    ...
    p.add_argument(
        ...
        help=(
            ...
            "(default) for normal editing and >=2 for layered RGBA generation."
        ),
    )
    p.add_argument(
        "--alpha_threshold",
...
def main() -> None:
    ...
    if args.prompt is not None:
        ...
        if args.compare:
            save_compare(out_path, images, _comparison_image(outputs), prompt)
        profiler.report()
        return
    ...
    for i, sample in enumerate(tqdm(samples, desc="Editing")):
        ...
        if think_text:
            ...
            think_path.write_text(think_text, encoding="utf-8")
        if args.compare:
            save_compare(sample_out, images, _comparison_image(outputs), prompt)
